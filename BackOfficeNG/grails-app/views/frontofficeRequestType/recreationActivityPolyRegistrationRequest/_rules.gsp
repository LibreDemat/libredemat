              ...
              <label class="required">
                ...
                <g:if test="${availableRules.contains('rulesAndRegulationsPolyAcceptance')}">
                  <a target="_blank" href="${createLink(controller:'localAuthorityResource', action:'rule', params:['requestTypeLabel':rqt.requestType.label, 'filename':'rulesAndRegulationsPolyAcceptance']).encodeAsXML()}"><span><g:message code="request.action.consult.rules" /></span></a>
                </g:if>
                <span><g:message code="raprr.property.rulesAndRegulationsPolyAcceptance.help" /></span>
              </label>
              <ul class="yes-no required ${rqt.stepStates['rules'].invalidFields.contains('rulesAndRegulationsPolyAcceptance') ? 'validation-failed' : ''}">
