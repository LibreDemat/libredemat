...
<html>
  <head>
     <title>${message(code:'ir.description')}</title>
    ...
    <g:if test="${specificConfigurationData}">
      <script type="text/javascript">
        zenexity.libredemat.irSpecificConfigurationData = ${specificConfigurationData};
      </script>
    </g:if>
    <script type="text/javascript">
  ...
  <body>
    ...
    <div id="request" class="main-box ${rqt.requestType.displayGroup == null ? '' : rqt.requestType.displayGroup.name}">
      <h2>
        ...
        <g:message code="ir.label" /> <g:message code="message.number" args="[rqt.id]" />
      ...
      <p><g:message code="request.duration.label" /><strong> : <g:message code="ir.duration.value" /></strong></p>
      <p>
      ...
      <div class="datas">
         <g:set var="requestTypeAcronym" value="ir" scope="request" />
      ...
      <g:set var="stepNumber" value="1"/>
        ...
        <li class="${currentStep == 'reason' ? 'current ' : ''}
          
            ${individual ? rqt.stepStates['reason-' + params.type].state : rqt.stepStates['reason'].state}
          "
          >
          <span class="number">${stepNumber++}</span>
          <a
            <g:if test="${currentStep != 'reason' && rqt.stepStates['reason'].state != 'unavailable'}">
              href="${createLink(controller:'frontofficeRequest', action : 'edit', params:['id':rqt.id,'currentStep':'reason'])}"
            </g:if>
          >
            
              <g:message code="${individual ? 'homeFolder.action.add' + org.apache.commons.lang.StringUtils.capitalize(params.type) : 'ir.step.reason.label'}" />
              ...
              <span class="help">
                <g:message code="request.step.message.${rqt.stepStates['reason' + (individual ? '-' + params.type : '')].state}" />
              </span>
