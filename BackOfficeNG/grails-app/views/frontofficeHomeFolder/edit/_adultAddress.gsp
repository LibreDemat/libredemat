<form id="adultAddress_${adult.id}" method="post" action="${g.createLink(action:'adult')}">
  ...
  <div id="adultAddress" class="address required ${invalidFields?.contains('address') ? 'validation-failed' : ''}">
    ...
    <input type="text" class="line2 required ${session.getAttribute('isAddresseReferentialCityRestriction') ? 'validate-streetNameReferential' : 'validate-streetName'} ${invalidFields?.contains('address.streetName') ? 'validation-failed' : ''}" maxlength="32" id="adultAddress_streetName" name="streetName" title="${message(code:'address.property.streetName.validationError')}" value="${adult.address?.streetName}" autocomplete="off" />
    ...
    <input type="text"
    class="line2 required validate-city ${invalidFields?.contains('address.city') ? 'validation-failed' : ''}"
    maxlength="32" id="adultAddress_city" name="city" title="${message(code:'address.property.city.validationError')}"
