...
import org.libredemat.business.request.ecitizen.HomeFolderModificationRequestData;
import org.libredemat.business.request.ecitizen.VoCardRequestData;
            ...
            if (homeFolder.isTemporary()) {
                ...
                for (RequestAction requestAction :
                    (List<RequestAction>)HibernateUtil.getSession().createSQLQuery(
                        "select * from request_action where request_id in (select id from request where home_folder_id = :homeFolderId and specific_data_class='org.libredemat.business.request.ecitizen.HomeFolderModificationRequestData') or request_id = :firstId order by date asc")
