    ...
    def widgets = [
      ...
      ,'family' :
          ...
          ${toGT('if (rqt.requestType.getStepAccountCompletion()){')}
            ...
            ${toGT('responsibleId = requester.getHomeFolder().getIndividuals().findAll{ it.getHomeFolderRoles(requester.getHomeFolder().getId()).findAll{ it.getRole() == org.libredemat.business.users.RoleType.HOME_FOLDER_RESPONSIBLE }.size() > 0 }.get(0).getId() ')}

            ${toGT('list = requester.getHomeFolder().getIndividuals().findAll{ !(it.getState().name.equals("Archived") || it.getState().name.equals("Invalid"))}.sort{a,b -> a.getClass() == org.libredemat.business.users.Adult.class ? (a.getId() == responsibleId ? -1 : 0) : 1 } ')}

            ${toGT('list.each { individual ->')}

              ${toGT('if (individual.getClass() == org.libredemat.business.users.Adult.class) {')}
              ...
              ${toGT('if (individual.getClass() == org.libredemat.business.users.Child.class) {')}
                ...
                ${toGT('requester.getHomeFolder().getIndividuals().each { indiv_2 -> ')}
                  ${toGT('if (indiv_2.getClass() == org.libredemat.business.users.Adult.class) {')}
