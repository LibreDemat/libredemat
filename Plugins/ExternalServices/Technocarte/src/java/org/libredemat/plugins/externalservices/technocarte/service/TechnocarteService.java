package org.libredemat.plugins.externalservices.technocarte.service;
...
import org.libredemat.Activite;
import org.libredemat.ArretAller;
import org.libredemat.CircuitAller;
import org.libredemat.EcoleDerogType;
import org.libredemat.EcoleSecteurType;
import org.libredemat.ListeActiviteDocument;
import org.libredemat.ListeEcoleDocument;
...
public class TechnocarteService extends ExternalProviderServiceAdapter implements IScholarBusinessProviderService {
    ...
    public void sendRequest(Request request) throws CvqException {
        ...
        parameters.addElement(new Parameter("code_appli", String.class, "Libredemat", null));
        try {
            Call call = new Call();
    ...
    public Map<String, Map<String, String>> getSchools(Child child) {
        ...
        if (externalHomeFolderService.getIndividualMapping(child, label) != null)
            externalChildId = externalHomeFolderService.getIndividualMapping(child, label).getExternalLibreDematId();
        logger.debug("Get externalLibreDematId for " + child.getFullName() + " - " + externalChildId);
        ...
        parameters.addElement(new Parameter("codeappli", String.class, "Libredemat", null));
        try {
            Call call = new Call();
            String encodingStyleURI = org.apache.soap.Constants.NS_URI_SOAP_ENC;
            call.setEncodingStyleURI(encodingStyleURI);
            ...
            if (soap_response.generatedFault()) {
                ...
                Parameter soap_result = soap_response.getReturnValue();
                // add a header to xml return 'xmlns="http://www.libredemat.org"'
                String xmlresult = soap_result.getValue().toString()
                    .replaceFirst("<ListeEcole>", "<ListeEcole xmlns=\"http://www.libredemat.org\">");
    ...
    private ListeActiviteDocument getActivities(String labelActivity, Request request, Child child) throws CvqObjectNotFoundException {
        ...
        try {
            ...
            if (soap_response.generatedFault()) {
                ...
                // add a header to xml return 'xmlns="http://www.libredemat.org"'
                String xmlresult = soap_result.getValue().toString()
                    .replaceFirst("<ListeActivite>", "<ListeActivite xmlns=\"http://www.libredemat.org\">");
